...
@section('content')
    {!! Form::model($products, ['method' => 'PATCH','route' => ['products.update', $products->product_id],'files' => true]) !!}
    <div class="row">
        <div class="col-md-8">
                    ...
                    <div class="row">
                        <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                                ...
                                {!! Form::text('product_name', $products->product_title, array('class' =>'form-control', 'required'=>'required')) !!}
                            </div>
                        </div>
                        <div class="col-xs-6 col-sm-6 col-md-6">
                            <div class="form-group">
                                <label>Sub Title</label>
                                {!! Form::text('sub_title', $products->sub_title, array('class' => 'form-control', 'required'=>'required')) !!}
                            </div>
                        </div>
                        <div class="col-xs-4 col-sm-4 col-md-4">
                            <div class="form-group">
                                <label>Category</label>
                                {{Form::select('sl_category',$category,$products->category_id,['class'=>'form-control'])}}
                            </div>
                        </div>
                        <div class="col-xs-4 col-sm-4 col-md-4">
                            <div class="form-group">
                                <label>Tags</label>
                                @can("edit-slug-product")
                                {!! Form::text('slug',$products->slug, array('class' => 'form-control','data-role'=>'tagsinput', 'required'=>'required')) !!}
                                @else
                                    {!! Form::text('slug',$products->slug, array('class' => 'form-control','readonly'=>'readonly')) !!}
                                @endcan
                            </div>
                        </div>
                        <div class="col-xs-4 col-sm-4 col-md-4">
                            <div class="form-group">
                                <label>Price</label>
                                {!! Form::text('custom_field', $products->custom_field, array('class' => 'form-control')) !!}
                            </div>
                        </div>
                        <div class="col-xs-6 col-sm-6 col-md-6 d-none">
                            <div class="form-group">
                                ...
                                <div class="selectgroup selectgroup-pills">
                                    @if($products->relatedLevel()->count()>0)
                                        @foreach($productLevel as $i=>$rows)
                                            @php($id=$products->relatedLevel()->where("pro_level_id",$rows->pro_level_id)->where("product_id",$products->product_id)->first())
                                            <label class="selectgroup-item">
                                                <input type="checkbox" name="product_level[]" value="{{$rows->pro_level_id}}" {{($rows->pro_level_id==$id['pro_level_id'])?"checked":""}} class="selectgroup-input">
                                    ...
                                    @else
                                        @foreach($productLevel as $i=>$rows)
                                            <label class="selectgroup-item">
                                                <input type="checkbox" name="product_level[]" value="{{$rows->pro_level_id}}" class="selectgroup-input">
                                                <span class="selectgroup-button">{{$rows->product_level}}</span>
                                            </label>
                                        @endforeach
                                    @endif
                        ...
                        <div class="col-xs-6 col-sm-6 col-md-6 d-none">
                            <div class="form-group">
                                <label>Annual</label>
                                {!! Form::text('annual', $products->annual, array('class' => 'form-control', 'required'=>'required')) !!}
                            </div>
                        </div>
                        <div class="col-xs-4 col-sm-4 col-md-4 d-none">
                            <div class="form-group">
                                ...
                                <div class="selectgroup selectgroup-pills">
                                    @php($list=explode(",",$products->type_id))
{{--                                    {{dd($list)}}--}}
                                    @if(count($list)>1)
                                        @php($arrID=$list)
                                    @else
                                ...
                                @foreach($product_type as $i=>$rows)
                                    @php($sel=array_search($rows->type_id,$list))
                                    <label class="selectgroup-item">
                                        <input type="checkbox" name="product_type[]" value="{{$rows->type_id}}" class="selectgroup-input" {{(false !== $sel)?"checked":""}}>
                        ...
                        <div class="col-xs-2 col-sm-2 col-md-2 d-none">
                            <div class="form-group">
                                ...
                                {!! Form::text('custom_field2', $products->custom_field1, array('class' => 'form-control')) !!}
                            </div>
                        </div>
                        <div class="col-xs-2 col-sm-2 col-md-2 d-none">
                            <div class="form-group">
                                <label>Custom field3</label>
                                {!! Form::text('custom_field3', $products->custom_field2, array('class' => 'form-control')) !!}
                            </div>
                        </div>
                        <div class="col-xs-2 col-sm-2 col-md-2 d-none">
                            <div class="form-group">
                                <label>Custom field4</label>
                                {!! Form::text('custom_field4', $products->custom_field3, array('class' => 'form-control')) !!}
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="form-group">
                                <label>Description</label>
                                {!! Form::textarea('description', $products->description, array('class' => 'form-control basictools','id'=>'basictools')) !!}
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select name="is_active" class="form-control">
                                                <option value="1" {{($products->is_active==1)?"selected":""}}>Publish</option>
                                                <option value="0" {{($products->is_active==0)?"selected":""}}>Private</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            {!! Form::text('order_by',$products->order_by, array('class' =>'form-control','required'=>'')) !!}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        <img
                                            ...
                                            src="{{!empty($products->images)?asset("{$products->images}"):"https://via.placeholder.com/246x356.png"}}"
                                            alt="preview">
                                        <div class="input-file input-file-image mt-2">
                                            <input type="text" value="{{$products->images}}" class="form-control form-control-file" id="thumb-col1" name="thumbcol1" accept="image/*">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        <img
                                            ...
                                            src="{{!empty($products->logo)?asset($products->logo):"https://via.placeholder.com/221x60.png"}}"
                                            alt="preview">
                                        <div class="input-file input-file-image mt-2">
                                            <input type="text" value="{{$products->logo}}" class="form-control form-control-file" id="thumb-col2" name="thumbcol2" accept="image/*">
